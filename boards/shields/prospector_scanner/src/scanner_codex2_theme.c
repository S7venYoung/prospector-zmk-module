...
static void battery_card(lv_obj_t *screen, int x, const char *side,
    ...
    lv_obj_t *card = panel(screen, x, 201, 122, 32, INK, MUTED, 10);
    lv_obj_t *side_label = text(card, side, &impact_16, MUTED);
    lv_obj_set_pos(side_label, 8, 8);
    *value = text(card, "--%", &impact_16, PAPER);
    lv_obj_set_pos(*value, 28, 8);
    *dot = lv_obj_create(card);
    plain(*dot, GREEN);
    lv_obj_set_size(*dot, 8, 8);
    lv_obj_set_pos(*dot, 106, 12);
    lv_obj_set_style_radius(*dot, LV_RADIUS_CIRCLE, 0);
    lv_obj_t *track = lv_obj_create(card);
    plain(track, PAPER);
    lv_obj_set_size(track, 38, 12);
    lv_obj_set_pos(track, 63, 10);
    lv_obj_set_style_radius(track, 3, 0);
    lv_obj_set_style_border_width(track, 1, 0);
    lv_obj_set_style_border_color(track, lv_color_hex(PAPER), 0);
    *fill = lv_obj_create(card);
    plain(*fill, GREEN);
    lv_obj_set_size(*fill, 4, 8);
    lv_obj_set_pos(*fill, 65, 12);
...
void scanner_codex2_theme_create(lv_obj_t *screen) {
    ...
    lv_obj_t *used = panel(screen, 14, 44, 122, 115, INK, YELLOW, 14);
    lv_obj_t *used_caption = text(used, "5 HOUR LEFT", &impact_16, PAPER);
    lv_obj_set_pos(used_caption, 8, 7);
    lv_obj_t *used_marks = text(used, "///", &impact_16, YELLOW);
    lv_obj_set_pos(used_marks, 101, 7);
    host_left = text(used, "--%", &impact_56, YELLOW);
    center(host_left, 4, 22, 114);
    lv_obj_t *week_rule = lv_obj_create(used);
    plain(week_rule, MUTED); lv_obj_set_size(week_rule, 104, 1); lv_obj_set_pos(week_rule, 8, 79);
    lv_obj_set_style_bg_opa(week_rule, LV_OPA_60, 0);
    lv_obj_t *week_caption = text(used, "7 DAY LEFT", &impact_16, PAPER);
    lv_obj_set_pos(week_caption, 8, 84);
    host_week = text(used, "--%", &impact_20, YELLOW);
    lv_obj_set_pos(host_week, 75, 82);
    for (int i = 0; i < 6; i++) {
        week_segments[i] = lv_obj_create(used);
        plain(week_segments[i], MUTED);
        lv_obj_set_size(week_segments[i], 15, 6);
        lv_obj_set_pos(week_segments[i], 8 + i * 17, 104);
        lv_obj_set_style_radius(week_segments[i], 1, 0);
    }

    lv_obj_t *tokens = panel(screen, 144, 44, 122, 115, INK, YELLOW, 14);
    lv_obj_t *tokens_caption = text(tokens, "TODAY TOTAL", &impact_16, PAPER);
    lv_obj_set_pos(tokens_caption, 8, 7);
    lv_obj_t *tokens_marks = text(tokens, "///", &impact_16, YELLOW);
    lv_obj_set_pos(tokens_marks, 101, 7);
    host_tokens = text(tokens, "--", &impact_48, PAPER);
    center(host_tokens, 2, 40, 118);

    lv_obj_t *bar = panel(screen, 14, 165, 252, 30, INK, MUTED, 10);
    ...
    lv_obj_set_pos(layer_value, 76, 2);
